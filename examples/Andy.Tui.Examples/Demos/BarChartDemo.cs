...
public static class BarChartDemo
{
    public static async Task Run((int Width, int Height) viewport, TerminalCapabilities caps)
    {
        ...
        try
        {
            ...
            {
                ("Alpha", 0.35), ("Beta", 0.7), ("Gamma", 0.5), ("Delta", 0.9), ("Epsilon", 0.2)
            };
            ...
            while (running)
            {
                ...
                while (Console.KeyAvailable)
                {
                    ...
                    {
                        // Randomize values
                        items = items.Select(it => (it.Label, Math.Clamp(it.Value + (rnd.NextDouble() - 0.5) * 0.2, 0, 1))).ToArray();
                        chart.SetItems(items);
                    }
                }
                ...
                b.DrawText(new DL.TextRun(2, 1, "Bar Chart — Space to randomize; ESC back; F2 HUD", new DL.Rgb24(200, 200, 50), null, DL.CellAttrFlags.Bold));
                var baseDl = b.Build();
                var wb = new DL.DisplayListBuilder();

                int height = Math.Min(items.Length, Math.Max(5, viewport.Height - 4));
                chart.Render(new L.Rect(2, 3, Math.Max(20, viewport.Width - 4), height), baseDl, wb);
