...
public static class StatusBarDemo
{
    public static async Task Run((int Width, int Height) viewport, TerminalCapabilities caps)
    {
        ...
        try
        {
            ...
            bar.SetColors(new DL.Rgb24(220,220,220), new DL.Rgb24(30,30,30));
            string mode = "NORMAL";
            ...
            while (running)
            {
                ...
                while (Console.KeyAvailable)
                {
                    ...
                    if (k.Key == ConsoleKey.I) mode = "INSERT";
                    if (k.Key == ConsoleKey.N) mode = "NORMAL";
                }
                ...
                b.DrawText(new DL.TextRun(2, 1, "Status Bar — I/N change mode; ESC back; F2 HUD", new DL.Rgb24(200,200,50), null, DL.CellAttrFlags.Bold));
                var baseDl = b.Build();

                var wb = new DL.DisplayListBuilder();
                bar.SetText($"{mode}", $"{DateTime.Now:HH:mm:ss}", "Ln 120, Col 8   UTF-8");
                bar.Render(new L.Rect(0, Math.Max(0, viewport.Height - 1), viewport.Width, 1), baseDl, wb);
