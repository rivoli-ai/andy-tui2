...
public static class KeyValueListDemo
{
    public static async Task Run((int Width, int Height) viewport, TerminalCapabilities caps)
    {
        ...
        try
        {
            ...
            kv.SetItems(new List<(string,string)>
            {
                ("Name","Andy.Tui"),
                ("Version","1.0.0"),
                ("License","MIT"),
            ...
            while (running)
            {
                ...
                b.DrawText(new DL.TextRun(2, 1, "Key–Value List — ESC back; F2 HUD", new DL.Rgb24(200,200,50), null, DL.CellAttrFlags.Bold));
                var baseDl = b.Build();

                var wb = new DL.DisplayListBuilder();
                kv.Render(new L.Rect(2, 3, Math.Max(0, viewport.Width - 4), Math.Max(0, viewport.Height - 5)), baseDl, wb);
