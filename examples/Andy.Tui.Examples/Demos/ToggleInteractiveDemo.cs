...
public static class ToggleInteractiveDemo
{
    public static async Task Run((int Width, int Height) viewport, TerminalCapabilities caps)
    {
        ...
        try
        {
            ...
            while (running)
            {
                ...
                b.DrawText(new DL.TextRun(2, 1, "Toggle — Space/Enter toggle; ESC/Q back; h HUD", new DL.Rgb24(200, 200, 50), null, DL.CellAttrFlags.Bold));
                var baseDl = b.Build();
                var wb = new DL.DisplayListBuilder();
                var toggle = new Andy.Tui.Widgets.Toggle(state, "WiFi");
                toggle.SetFocused(true);
                toggle.Render(new Andy.Tui.Layout.Rect(2, 3, 16, 1), baseDl, wb);
