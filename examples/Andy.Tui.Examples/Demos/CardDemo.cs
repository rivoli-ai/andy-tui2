...
public static class CardDemo
{
    public static async Task Run((int Width, int Height) viewport, TerminalCapabilities caps)
    {
        ...
        {
            bool running = true;
            var card = new Andy.Tui.Widgets.Card();
            card.SetTitle("Card Title");
            card.SetBody("This is a simple card with header, body, and optional footer.\nIt demonstrates content wrapping and placement.");
            card.SetFooter("Footer text");
            ...
            while (running)
            {
                ...
                b.DrawText(new DL.TextRun(2, 1, "Card — ESC back; F2 HUD", new DL.Rgb24(200,200,50), null, DL.CellAttrFlags.Bold));
                var baseDl = b.Build();

                var wb = new DL.DisplayListBuilder();
                int w = Math.Max(20, viewport.Width - 6);
                int h = Math.Max(6, viewport.Height - 6);
                card.Render(new L.Rect(3, 3, w, h), baseDl, wb);
