...
public static class TerminalText
{
    ...
    // ---------------------------------------------------------------------
    // Trust boundary for application-supplied text.
    //
    // Any string that ultimately reaches the terminal must be plain display
    // text: it must never carry terminal control characters. The ANSI encoder
    // writes run text verbatim, so a control byte embedded in ordinary content —
    // a log line, Markdown, a chat message, a filename, or a network payload —
    // would be interpreted as a terminal command (move the cursor, clear the
    // screen, ring the bell, open an OSC 8 hyperlink, write the clipboard via
    // OSC 52, or set the window title). Trusted terminal control flows through
    // typed operations (coordinates and SGR attributes) emitted by the encoder
    // itself, never smuggled through text. The methods below rewrite control
    // characters found in ordinary text into visible, inert, single-cell
    // placeholders so untrusted content cannot escape the text plane.
    // ---------------------------------------------------------------------
